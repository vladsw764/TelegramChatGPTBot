...
class ChatGPT3TelegramBot:
    ...
    async def help(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        """
        await update.message.reply_text("Ви у головному меню:\n\n"
                                        "/reset - Оновлює бесіду\n\n"
                                        "[Будь яке повідомлення] - Відправляє ваше повідомлення до AI\n\n"
                                        "/help - Меню помічника\n\n"
                                        "/more_info - Більше інформації про нашого бота\n\n",
                                        disable_web_page_preview=True)



    async def more_info(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        await update.message.reply_text("Цей бот розроблений з метою допомогти користувачам швидко та легко знаходити відповіді на запитання, отримувати необхідну інформацію та розв'язувати проблеми через автоматизований діалог.\n\n"
                                        "/reset - Оновлює бесіду\n\n"
                                        "[Будь яке повідомлення] - Відправляє ваше повідомлення до AI\n\n"
                                        "/help - Меню помічника\n"
                                        , disable_web_page_preview=True)
    ...
    async def prompt(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        logging.info(f'New message received from user {update.message.from_user.send_action}')
        chat_id = update.effective_chat.id
    ...
    def run(self):
        ...
        application.add_handler(CommandHandler('more_info', self.more_info))
